...
const ActiveMissions = () => {
  ...
  const totalProgress = sortedTrackers.length > 0 
    ? Math.round(sortedTrackers.reduce((acc, t) => acc + Math.min(100, (t.daycount / t.duration) * 100), 0) / sortedTrackers.length)
    : 0;
    ...
    <AuthLayout>
      ...
      <div className="min-h-screen bg-[#0f1015] text-[#e2e8f0] p-4 pb-20 relative overflow-hidden flex justify-center">
        ...
        <div className="w-full max-w-md relative z-10 font-secondary mt-4">
            ...
            <Link to={/missions/\} className="block relative bg-[#15161e] rounded-xl mb-6 overflow-hidden border border-[#2a2b36] hover:border-[#a855f7]/50 transition-colors group animate-fade-in-up shadow-xl shadow-black/50" style={{ animationDelay: '0.2s' }}>
              ...
              <div className="relative p-5 z-10">
                ...
                <p className="text-xs text-gray-400 leading-relaxed mb-6 font-secondary pr-4 line-clamp-3">
                  {legendary.description || "Conquer the high-rank dungeon in the void dimension. Extract the shadows of fallen commanders to build your army."}
                </p>
                ...
                <div className="h-2 w-full bg-[#0a0a0f] rounded-full overflow-hidden border border-white/10">
                  <div 
                    className="h-full bg-gradient-to-r from-[#a855f7] to-[#d946ef] rounded-full shadow-[0_0_10px_rgba(217,70,239,0.8)] transition-all duration-700 ease-out"
                    style={{ width: \\%\ }}
                  ></div>
                </div>
              </div>
          ...
          {secondary && (
            <Link to={/missions/\} className="block relative bg-[#15161e] rounded-xl p-4 mb-8 border border-[#2a2b36] hover:border-[#a855f7]/30 transition-colors group animate-fade-in-up shadow-lg shadow-black/40" style={{ animationDelay: '0.3s' }}>
              <div className="flex justify-between items-center mb-3">
                <div className="bg-[#a855f7] text-black text-[10px] font-black px-2 py-0.5 rounded font-primary tracking-widest">
                  RANK {secondary.rank}
                </div>
              ...
              <p className="text-xs text-gray-400 mb-5 leading-snug line-clamp-2">
                {secondary.description || "Analyze the combat patterns of elite magic beasts to refine your skills."}
              </p>
              ...
              <div className="h-1 w-full bg-[#0a0a0f] rounded-full overflow-hidden mb-3 border border-white/5">
                <div 
                  className="h-full bg-[#d946ef] rounded-full transition-all duration-700 ease-out shadow-[0_0_8px_rgba(217,70,239,0.6)]"
                  style={{ width: \\%\ }}
                ></div>
              </div>
          ...
          {/* Bottom Module (Loot Probability) */}
...
'''
with open('src/pages/Activemissions.jsx', 'w', encoding='utf-8') as f:
    f.write(content)
